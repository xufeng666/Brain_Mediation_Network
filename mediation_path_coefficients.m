...
function [paths, sterrs,abetas,bbetas,cpbetas,cbetas, residm, residy, residy2, Vxm, Vmy, Vxy] = ols_mediation_paths_withstes(x, y, m, intcpt, n, logistic_Y, varargin)
    ...
    nmediators = size(m, 2);   %M的列  单中介
    
    % Is X related to the mediator?
    % Eq. 2, a = M ~ X  residm 残差 xtxi是 xi平方和的逆 px是xtxi*x'
    ...
    aste = aste(1, :); % same as above.  每个回归的第一个系数
    vara = vara(1, :);
    ...
    % group model
    if ~logistic_Y
        abvar = (b.^2 .* vara) + (a.^2 .* varb) + (vara .* varb);   %#Aroian test equation   z-value = a*b/SQRT(b2*sa2 + a2*sb2 + sa2*sb2)
        abste = abvar .^ .5;
    else
        % Iacobucci(2012) eq(5)
...
function [beta_vec, stebeta, varbeta, xtxi, px, resid, V] = regression(x,y,intcpt,n,xtxi,px,varargin)
    ...
    % to a different regression model.
%beta_vec、varbeta 和 stebeta 的每个 col，以及 V 的每个单元格，指的是不同的回归模型。
    ...
    % is a separate regression.  步骤 1：找到每个 y col 的 OLS 解决方案: beta_vec 的每一列都是一个单独的回归。
    nregressions = size(y, 2);

    beta_vec = px * y;                                % Beta values 相关系数
    resid = y - X * beta_vec;                         % Residuals  残差
    ...
    if arorder > 0
        ...
        for i = 1:nregressions
            ...
            varbeta(:, i) = diag(V{i});                    % var of coefficients  变量的系数
        end
        stebeta = varbeta.^.5;
    end
